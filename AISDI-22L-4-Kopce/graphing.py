...
def generate_and_save_graph(name, description, times_2, times_3, times_4):
    keys = [i * 10000 for i in range(1, 11)]
    title = ""
    for letter in name:
        if letter != "_":
            title += letter
            ...
            title += " "
    plt.plot(
        keys, times_2, 'o-', label="2-ary", markersize=3, color='b'
    )
    plt.plot(
        keys, times_3, 'o-', label="3-ary", markersize=3, color='y'
    )
    plt.plot(
        keys, times_4, 'o-', label="4-ary", markersize=3, color='g'
    )
    plt.legend()
